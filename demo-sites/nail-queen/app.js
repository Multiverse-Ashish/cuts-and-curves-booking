...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const currency = window.SALON_CURRENCY || '₹';
  const taxName = window.SALON_TAX_NAME || 'GST (18%)';
  const taxRate = typeof window.SALON_TAX_RATE !== 'undefined' ? window.SALON_TAX_RATE : 0.18;
  ...
  const openDetailModal = (btn) => {
    ...
    const gst = Math.round(price * taxRate);
    const total = price + gst;

    if (modalPrice) modalPrice.textContent = currency + total;
    if (modalBasePrice) modalBasePrice.textContent = currency + price;
    if (modalGstAmount) modalGstAmount.textContent = currency + gst;
  ...
  const populateServices = () => {
    ...
    list.forEach(s => {
      ...
      opt.value = s.n;
      opt.textContent = `${s.n} — ${currency}${s.p}`;
  ...
  const populateCourses = () => {
    ...
    list.forEach(c => {
      const opt = document.createElement('option');
      opt.value = c.n;
      opt.textContent = `${c.n} — ${currency}${c.p} (${c.d} Wks)`;
  ...
  const updateHpInvoice = () => {
    ...
    const gst  = Math.round(base * taxRate);
    hpInvBase.textContent  = currency + base;
    hpInvGst.textContent   = currency + gst;
    hpInvTotal.textContent = currency + (base + gst);
  };
  ...
  if (form) {
    ...
    const updateInvoice = () => {
      ...
      const gst  = Math.round(base * taxRate);
      invBase.textContent  = currency + base;
      invGst.textContent   = currency + gst;
      invTotal.textContent = currency + (base + gst);
    };
...
Please confirm my slot. Thank you! 🙏`;
  ...
  // Automatic Gallery Carousel Slideshow (Horizontal translate slider)
  document.querySelectorAll('.gallery-carousel').forEach(carousel => {
    const track = carousel.querySelector('.gallery-carousel-slides');
